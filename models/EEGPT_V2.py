...
class CAE(nn.Module):
    ...
    def forward(self, x, 
        ...
        for idx in range(2):#
            ...
            (z2,t2), pz_m, attn_mask_enc2 = \
                            self.forward_encoder_momentum(  input_y, 
                                                            chan_pos_y, 
                                                            time_pos_y,
                                                            chan_pos_sum, 
                                                            time_pos_sum)
    ...
    def forward_encoder(self, 
        ...
        z_inter, (z,t), attn_mask_enc = self.model.encoder(input_x, 
                                                    ...
                                                    chan_pos_sum, 
                                                    time_pos_sum)
        # z_chan0, z_time0, z_both0 = z_inter
        
        pz   = self.projection_head(z)
        pz_y = self.prediction_head(pz)
        # z_chan, z_time, z_both = torch.split(pz_y, [z_chan0.shape[1]*z_chan0.shape[2], \
        #                                             z_time0.shape[1]*z_time0.shape[2], \
        #                                             z_both0.shape[1]*z_both0.shape[2]], dim=1)
        # z_chan, z_time, z_both = z_chan.reshape(z_chan0.shape[:-1]+(z_chan.shape[-1],)), \
        #                          z_time.reshape(z_time0.shape[:-1]+(z_time.shape[-1],)), \
        #                          z_both.reshape(z_both0.shape[:-1]+(z_both.shape[-1],))
        
        # return (z,t), (z_chan, z_time, z_both), attn_mask_enc
    ...
    def forward_encoder_momentum(self, 
        ...
        with torch.no_grad():
            ...
            # z_chan, z_time, z_both = torch.split(pz_m, [z_chan0.shape[1]*z_chan0.shape[2], \
            #                                             z_time0.shape[1]*z_time0.shape[2], \
            #                                             z_both0.shape[1]*z_both0.shape[2]], dim=1)
            # z_chan, z_time, z_both = z_chan.reshape(z_chan0.shape[:-1]+(z_chan.shape[-1],)), \
            #                          z_time.reshape(z_time0.shape[:-1]+(z_time.shape[-1],)), \
